...
describe('TrainingSession', () => {
  ...
  it('chrome collapses cleans into appliance_wash', () => {
    ...
    session.notifyRemoved('filter_top');
    expect(
      session.buildChromeSteps().find((s) => s.stepId === 'appliance_wash')?.status,
    ).toBe('current');
    ...
    expect(
      session.buildChromeSteps().find((s) => s.stepId === 'install_filter_top')?.status,
    ).toBe('current');
  });
  ...
  it('isApplianceWashReady false when no clean spots', () => {
    ...
    expect(session.buildChromeSteps().some((s) => s.stepId === 'appliance_wash')).toBe(
      false,
    );
  });
