...
test("findNearest among equal priority picks closer", () => {
  clearInteractables();
  registerInteractable(stub("a", [0.2, 0, 0], { priority: INSTALLED_PICK_PRIORITY }));
  registerInteractable(stub("b", [0.1, 0, 0], { priority: INSTALLED_PICK_PRIORITY }));
...
test("findNearest ignores out-of-radius even with high priority", () => {
  clearInteractables();
  registerInteractable(stub("far_sop", [0.5, 0, 0], { priority: SOP_PICK_PRIORITY, radius: 0.18 }));
  registerInteractable(
