...
1. Add `@nestjs/serve-static`; wire `ServeStaticModule.forRootAsync` with `exclude: ['/api/{*any}']` (not `/api*` — path-to-regexp v8); resolve `public` via cwd/`STATIC_ROOT` at boot.
...
- [x] Implement + check green (ServeStatic exclude fix, e2e, build:docker smoke)
