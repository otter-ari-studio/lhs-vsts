...
`heyongqi10` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/heyongqi10/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/heyongqi10/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee heyongqi10` to
...
python3 ./.trellis/scripts/task.py archive 00-join-heyongqi10
```
